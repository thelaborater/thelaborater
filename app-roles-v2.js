...
(() => {
  ...
  function portraitSrc(character, text="") {
    const t=String(text||"");
    if(character==="corn"){
      if(/不在|失蹤|找不到|怎麼|啊啊|拜託|難過|緊張|嚇|苦惱|真的嗎|怎⋯|唔/.test(t)) return "assets/characters/corn/panic.png";
      if(/謝謝|感謝|高興|喜歡爆抱|哦～|原來如此|太感謝|沒問題|找到|好像有什麼/.test(t)) return "assets/characters/corn/happy.png";
      return "assets/characters/corn/idle.png";
    }
    if(character==="churro"){
      if(/電影票|票給|給你們|買票/.test(t)) return "assets/characters/churro/ticket.png";
      if(/後面還有很多人|快|所以呢|你還想要什麼/.test(t)) return "assets/characters/churro/hurry.png";
      if(/沒印象|誰知道|唉|⋯⋯|不知道/.test(t)) return "assets/characters/churro/hmph.png";
      if(/等一下|再見/.test(t)) return "assets/characters/churro/goodbye.png";
      if(/下一位/.test(t)) return "assets/characters/churro/next.png";
      return "assets/characters/churro/welcome.png";
    }
    if(character==="hotdog"){
      if(/驗票|電影票|買票|票根|票券/.test(t)) return "assets/characters/hotdog/ticket.png";
      if(/我知道了|希望可以幫助|好厲害/.test(t)) return "assets/characters/hotdog/praise.png";
      if(/話雖如此|你知道|這倒是沒有|不過|那個|為了防止|電影結束/.test(t)) return "assets/characters/hotdog/explain.png";
      if(/哈哈|哎呀|呀勒呀勒|欸欸/.test(t)) return "assets/characters/hotdog/coax.png";
      if(/不要騙|下次見/.test(t)) return "assets/characters/hotdog/goodbye.png";
      return "assets/characters/hotdog/welcome.png";
    }
    return "";
  }

  function portraitImg(character, text="", alt="") {
    const src = portraitSrc(character,text);
    return `<img src="./${src}?v=roles-v2" alt="${esc(alt)}" draggable="false"
      onerror="this.dataset.failed='true';this.style.outline='3px solid #ff4f4f';this.alt='角色圖片載入失敗：${esc(alt)}'">`;
  }
      ...
      app.innerHTML = `
        <section class="conversation-scene ${character}">
          ...
          <div class="character character-left ${cornSpeaking?"active":"dim"} ${tear}">
            <div class="portrait corn">${portraitImg("corn", text, "寇恩")}</div><b>寇恩</b>
          </div>
          <div class="character character-right ${mainSpeaking?"active":"dim"}">
            <div class="portrait ${character}">${portraitImg(character, text, mainName)}</div><b>${mainName}</b>
          </div>` : `
  ...
  function renderClueMenu() {
    app.innerHTML = `
      <section class="conversation-scene corn-menu">
        <div class="character character-center active"><div class="portrait corn">${portraitImg("corn", "", "寇恩")}</div><b>寇恩</b></div>
  ...
  function renderChurroRepeat() {
    runDialogue(D.secondTheater.churroRepeat, ()=>{
      ...
      <section class="conversation-scene churro">
        <div class="character character-center active"><div class="portrait churro">${portraitImg("churro", "", "丘洛")}</div><b>丘洛</b></div>
        <div class="choice-panel">
